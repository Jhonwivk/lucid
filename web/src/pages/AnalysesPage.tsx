...
export function AnalysesPage() {
  ...
  function openComposer() {
    ...
    window.setTimeout(() => document.getElementById('new-analysis-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0)
  }
      ...
      <div className="shell">
        ...
        <section className="concept-layout">
          ...
          <aside className="decision-brief" aria-label={isZh ? '决策简报' : 'Decision brief'}>
            ...
            <div className="brief-section"><div className="brief-section-title">{isZh ? '下一步行动' : 'Next action'}</div><p className="next-action">{isZh ? '选择一个完整案例，或输入自己的决策问题开始。' : 'Choose a runnable case or enter your own decision question to begin.'}</p><button className="btn btn-primary brief-action" type="button" onClick={openComposer}>{isZh ? '添加材料' : 'Add materials'} →</button></div>
            <div className="brief-section brief-resources"><div className="brief-section-title">{isZh ? '常用资源' : 'Common resources'}</div><div className="resource-list">{templates.state === 'ready' ? templates.templates.slice(0, 3).map((template) => <button key={template.id} type="button" className="resource-chip" onClick={() => void onUseTemplate(template.id)}><span>{template.category === 'portfolio' ? '◈' : '▦'}</span><span><strong>{isZh ? template.name_zh : template.name_en}</strong><small>{template.source_count} {t('sources')}</small></span></button>) : <span className="muted">{t('templatesLoading')}</span>}</div></div>
            {composerOpen ? <form id="new-analysis-form" className="composer brief-composer" onSubmit={onCreate}><div className="composer-heading"><h3>{t('startAnalysis')}</h3><button className="btn btn-ghost" type="button" onClick={() => setComposerOpen(false)} aria-label={isZh ? '关闭' : 'Close'}>×</button></div><p>{t('startAnalysisHint')}</p><label className="field"><span>{t('title')}</span><input value={title} onChange={(event) => setTitle(event.target.value)} required maxLength={200} placeholder={t('titlePlaceholder')} /></label><label className="field"><span>{t('decisionQuestion')}</span><textarea value={question} onChange={(event) => setQuestion(event.target.value)} required maxLength={8000} placeholder={t('decisionQuestionPlaceholder')} /></label><label className="field"><span>{t('pasteTextOptional')}</span><textarea value={pasted} onChange={(event) => setPasted(event.target.value)} maxLength={20000} /></label><div className={`drop-zone${dragging ? ' over' : ''}`} onDragOver={(event: DragEvent) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(event: DragEvent) => { event.preventDefault(); setDragging(false); const next = Array.from(event.dataTransfer.files); if (next.length) setFiles(next) }}><label className="field"><span>{t('filesOptional')}</span><input type="file" multiple accept=".txt,.md,.markdown,.pdf,.csv,.xlsx,.png,.jpg,.jpeg,.json,.docx,.pptx,text/plain,text/markdown,text/csv,application/pdf,application/json,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.presentationml.presentation,image/png,image/jpeg" onChange={(event) => setFiles(event.target.files ? Array.from(event.target.files) : [])} /></label><p className="muted">{t('dropFiles')}. {t('questionOnlyOk')}{files.length ? ` · ${files.length} ${t('filesSelected')}` : ''}</p></div>{createError ? <p className="muted" role="alert">{createError}</p> : null}<button className="btn btn-primary" type="submit" disabled={creating || !title.trim() || !question.trim()}>{creating ? t('creating') : t('createAndContinue')}</button></form> : null}
          </aside>
        </section>
